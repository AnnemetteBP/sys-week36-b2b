...
namespace Customer
{
    public class Customer
    {
        ...
        public async void Start()
        {
            ...
            using (IBus bus = RabbitHutch.CreateBus("host=stingray-01.rmq.cloudamqp.com;virtualHost=dslmjuud;username=dslmjuud;password=8XLswZaA5FI2JxQOiT0J8BrCgFj2STVn"))
            {
                // Listen to reply messages from the Retailer (use Topic Based Routing).
                // WRITE CODE HERE!
                bus.PubSub.Subscribe<OrderReplyMessage>("Customer" + customerID, HandleOrderEvent,
                    );

                // Send an order request message to the Retailer (use a point-to-point channel).
                // WRITE CODE HERE!
                var message = new OrderRequestMessage { Country = "DK", CustomerId=1, OrderId=1, ProductId=1 };
                bus.SendReceive.Send<OrderRequestMessage>("customerToRetailerQueue", message);
